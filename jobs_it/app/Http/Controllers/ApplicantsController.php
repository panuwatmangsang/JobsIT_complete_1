...
class ApplicantsController extends Controller
{
    ...
    public function applicants_myjobs(Request $request)
    {
        ...
        if ($request->input('type') == 'all') {
            ...
            $all = MyJobs::all();
            $noti_count = MyJobs::query()->where('a_id', 'LIKE', '4')->orWhere('a_id', 'LIKE', '5')->count();
            $noti_count_box = MyJobs::query()->where('a_id', 'LIKE', '4')->orWhere('a_id', 'LIKE', '5')->get();
    ...
    public function change_password(Request $request, $app_id)
    {
        // dd('ok');
        // $user = Auth::user();
        // $user = Applicants::get($request->app_name);
        ...
        $appInfo = Applicants::where('app_password', 'LIKE', '$request->app_name');
        // $user = $request->session()->put('LoggedApp', $appInfo->app_name);
        // dd($user);

        // $hashedPassword = Auth::user()->password;

        if (Hash::check($request->current_password, $appInfo)) {

            if (!Hash::check($request->new_password, $appInfo)) {

                $password_edit = Applicants::find($app_id);
                $password_edit->app_password = bcrypt($request->new_password);
                Applicants::where('app_id', Applicants::find($app_id))->update(array('app_password' =>  $password_edit->app_password));
